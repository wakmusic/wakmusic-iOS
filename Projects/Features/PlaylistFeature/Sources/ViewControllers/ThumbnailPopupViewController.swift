...
final class ThumbnailPopupViewController: BaseReactorViewController<ThumbnailPopupReactor> {
    ...
    private let titleLabel: WMLabel = WMLabel(text: "표지", textColor: DesignSystemAsset.BlueGrayColor.blueGray900.color, font: .t2(weight: .bold), alignment: .center)
...
extension ThumbnailPopupViewController {
    func createDataSoruce() -> UITableViewDiffableDataSource<Int, ThumbnailOptionModel> {
        return UITableViewDiffableDataSource<Int, ThumbnailOptionModel>(tableView:tableView) { tableView, indexPath, itemIdentifier in
