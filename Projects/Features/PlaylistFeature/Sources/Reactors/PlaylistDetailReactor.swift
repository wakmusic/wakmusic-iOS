...
public final class PlaylistDetailReactor: Reactor {
    ...
    public init(
        ...
        self.initialState = .init(
            dataSource: [], backupDataSource: [], header: PlayListHeader(title: "", songCount: "", image: "", version: 0), selectedItemCount: 0)
    }
    ...
    public func reduce(state: State, mutation: Mutation) -> State {
        ...
        case .itemMoved((let sourceIndex, let destinationIndex)): // TODO: remove insert
            break
        case .updateData(let metadata):
...
private extension PlaylistDetailReactor {
    func fetchData() -> Observable<Mutation> {
        return fetchPlayListDetailUseCase.execute(id: key, type: type)
            .catchAndReturn(PlayListDetailEntity(key: key, title: "", songs: [], image: "", image_square_version: 0, image_round_version: 0, version: 0)
            )
            .asObservable()
            .map{ [weak self] result in
                
                guard let self else {
                    return PlaylistMetaData(list: [], header: PlayListHeader(title: "", songCount: "", image: "", version: 0))
                }
                return  PlaylistMetaData(list: [PlayListDetailSectionModel(model: 0, items: result.songs)],
