...
public extension RecommendPlayListView {
    ...
    private func setupView(){
        ...
        collectionView.register(UINib(nibName:"RecommendPlayListCell", bundle: Bundle.module),
    ...
    static func getViewHeight(model: [RecommendPlayListEntity]) -> CGFloat {
        ...
        if model.count == 1 {
            ...
            if remain == 0 {
                ...
                return base + (CGFloat(mok) * itemHeight) + (CGFloat(remain) * itemHeight) + (CGFloat(mok-1) * spacing) + (CGFloat(remain) * spacing)
            }
        }
    }
}

extension RecommendPlayListView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
