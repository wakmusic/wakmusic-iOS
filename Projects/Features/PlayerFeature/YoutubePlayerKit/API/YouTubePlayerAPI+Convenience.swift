        ...
        .flatMap { _ in
            Future { [weak self] promise in
                self?.getCurrentTime { result in
                    guard case let .success(currentTime) = result else {
                        return
                    }
        ...
        .flatMap { _ in
            Future { [weak self] promise in
                self?.getVideoLoadedFraction { result in
                    guard case let .success(fraction) = result else {
                        return
                    }
            ...
            .flatMap { _ in
                Future { [weak self] promise in
                    self?.getPlaybackMetadata { result in
                        guard case let .success(playbackMetadata) = result else {
                            return
                        }
            ...
            .flatMap { _ in
                Future { [weak self] promise in
                    self?.getDuration { result in
                        guard case let .success(duration) = result else {
                            return
                        }
